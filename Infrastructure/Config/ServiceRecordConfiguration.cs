...
public class ServiceRecordConfiguration : IEntityTypeConfiguration<ServiceRecord>
{
    public void Configure(EntityTypeBuilder<ServiceRecord> builder)
    {
        ...
        builder.HasOne(sr => sr.Pet).WithMany(p => p.ServiceRecords).HasForeignKey(sr => sr.PetId).OnDelete(DeleteBehavior.Cascade);
        ...
        builder.HasOne(sr => sr.Appointment).WithMany(a => a.ServiceRecords).HasForeignKey(sr => sr.AppointmentId).OnDelete(DeleteBehavior.SetNull);
        builder.HasOne(sr => sr.Staff).WithMany(a => a.ServiceRecord).HasForeignKey(sr => sr.StaffId).OnDelete(DeleteBehavior.SetNull);
        builder.HasMany(sr => sr.Steps).WithOne(step => step.ServiceRecord).HasForeignKey(step => step.ServiceRecordId).OnDelete(DeleteBehavior.Cascade);
